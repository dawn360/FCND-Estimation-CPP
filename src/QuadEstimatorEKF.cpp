...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // calculate the rotation from body frame to inertial/global frame for accel 
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  gPrime(1,4) = dt;
  gPrime(2,5) = dt;
  gPrime(3,6) = (RbgPrime(0)*accel).sum()*dt; //RbgPrime[0:]ut[0:3]dt
  gPrime(4,6) = (RbgPrime(1)*accel).sum()*dt; //RbgPrime[1:]ut[0:3]dt
  gPrime(5,6) = (RbgPrime(2)*accel).sum()*dt; //RbgPrime[2:]ut[0:3]dt
